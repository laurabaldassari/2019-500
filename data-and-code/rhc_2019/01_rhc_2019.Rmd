...
-----: | ------: | -----: | --------- | -------: | -------------------------
None | `r round_half_up(rubin1.unadj,1)` | `r round_half_up(rubin2.unadj,2)` | Terrible | -- |  No Matching
1 | `r round_half_up(rubin1.m1, 1)` | `r round_half_up(rubin2.m1, 2)` | Not Good | 2,184 | 1:1 greedy w/o repl
2 | `r round_half_up(rubin1.m2, 1)` | `r round_half_up(rubin2.m2, 2)` | Terrible | 1,775 | 1:2 greedy w/o repl
3 | `r round_half_up(rubin1.m3, 1)` | `r round_half_up(rubin2.m3, 2)` | Good | 2,184 | Genetic Search 1:1 w/o repl
4 | `r round_half_up(rubin1.m4, 1)` | `r round_half_up(rubin2.m4, 2)` | Not Good | 2,184 | 1:1 greedy with repl
5 | `r round_half_up(rubin1.m5, 1)` | `r round_half_up(rubin2.m5, 2)` | Great | 1,563 | 1:1 caliper with repl
6 | `r round_half_up(rubin1.m6, 1)` | `r round_half_up(rubin2.m6, 2)` | Good | 2,184 | 1:2 greedy with repl
...
The `rbounds` package can evaluate binary outcomes using the `binarysens` and `Fishersens` functions. The `binarysens` function works directly on a matched sample as developed using the Matching package. For example, in our third match, based on the genetic search algorithm with 1:1 matching without replacement, this is `match3`.

```{r}
binarysens(match3, Gamma = 2, GammaInc = 0.1)
```

At a 5% significance level, we retain significance up to a hidden bias of $\Gamma$ = 1.2, but not at $\Gamma$ = 1.3. See the `toy_2019` example discussed in Class 4, as well as Chapter 9 of Rosenbaum's *Observation and Experiment* for more details on interpretation of this result.

We can also use this approach with other Matching results, including Match 6, which is a 1:2 match with replacement.

```{r}
binarysens(match6, Gamma = 3, GammaInc = 0.25)
```

Here, at a 5% significance level, we retain significance up to a hidden bias of $\Gamma$ = 2.5, but not at $\Gamma$ = 2.75.
